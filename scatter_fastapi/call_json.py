...
class Call_Sk:
    ...
    areainfo = {
        "롯데월드": {"datetime": "", "congestion_level": 0},
        "방이동먹자골목": {"datetime": "", "congestion_level": 0},
        "에비뉴엘월드타워점": {"datetime": "", "congestion_level": 0},
        "롯데월드몰": {"datetime": "", "congestion_level": 0},
        "올림픽공원": {"datetime": "", "congestion_level": 0}
    }
    
    def sk_open_api(self, url: str):
        response = requests.get(url)
        hotspots_data = response.content.decode('utf-8')
        return json.loads(hotspots_data)

    # ... [Other methods follow in the same style] ...

    def rename_level(self, congestion: float) -> str:
